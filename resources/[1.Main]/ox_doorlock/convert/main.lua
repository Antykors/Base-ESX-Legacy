				...
				if next(Config.DoorList) then
					...
					for k, door in pairs(Config.DoorList) do
						...
						if qb then
							...
							local groups = door.authorizedJobs and door.authorizedFaction or {}

							if door.authorizedFaction then
								for faction, grade in pairs(door.authorizedFaction) do
									groups[faction] = grade
								end
							end

							door.authorizedJobs = next(groups) and groups
							door.authorizedFaction = next(groups) and groups
						...
						local data = {
							...
							groups = (door.authorizedJobs) and (door.authorizedFactions),
							items = door.items,
