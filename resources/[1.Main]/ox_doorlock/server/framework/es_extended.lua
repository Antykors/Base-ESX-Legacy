...
function IsPlayerInGroup(player, filter)
    ...
    if type == 'string' then
        ...
        if player.faction.name == filter then
        ...
        if tabletype == 'hash' then
            ...
            local grade = filter[player.faction.name]

            if grade and grade <= player.job.grade then
                return player.job.name, player.job.grade
            end
            if grade and grade <= player.faction.grade then
                return player.faction.name, player.faction.grade
            end
        elseif tabletype == 'array' then
            for i = 1, #filter do
                ...
                if player.faction.name == filter[i] then
                    return player.faction.name, player.faction.grade
                end
            end
        end
