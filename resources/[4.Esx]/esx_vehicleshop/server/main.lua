...
RegisterNetEvent('esx_vehicleshop:setVehicleOwnedPlayerJob')
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerJob', function(playerId, vehicleProps, model, label)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
	...
	function(id)
		...
		function(rowsChanged)
			if rowsChanged == 1 then
				MySQL.insert('INSERT INTO owned_vehicles (owner, plate, job, vehicle) VALUES (?, ?, ?, ?)', {xTarget.job.name, vehicleProps.plate, xTarget.job.name, json.encode(vehicleProps)},
				function(id)
					xPlayer.showNotification(TranslateCap('vehicle_set_owned', vehicleProps.plate, xTarget.getName()))
					xTarget.showNotification(TranslateCap('vehicle_belongs', vehicleProps.plate))
				end)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		exports['gflp10-carkeys']:AddCarkey(source, plate)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyjobVehicle', function(source, cb, model, plate)
	local xPlayer = ESX.GetPlayerFromId(source)
	local modelPrice = getVehicleFromModel(model).price
	...
	if modelPrice and xPlayer.getMoney() >= modelPrice then
		...
		MySQL.insert('INSERT INTO owned_vehicles (owner, plate, job, vehicle) VALUES (?, ?, ?, ?)', {xPlayer.job.name, plate, xPlayer.job.name, json.encode({model = joaat(model), plate = plate})
		}, function(rowsChanged)
			xPlayer.showNotification(TranslateCap('vehicle_belongs', plate))
			ESX.OneSync.SpawnVehicle(joaat(model), Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading,{plate = plate}, function(vehicle)
				Wait(100)
