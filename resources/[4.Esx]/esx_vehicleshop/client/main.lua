...
function OpenShopJobMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					...
					ESX.TriggerServerCallback('esx_vehicleshop:buyjobVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
		...
		CurrentAction     = 'shop_job_menu'
		CurrentActionMsg  = TranslateCap('shop_job_menu')
		CurrentActionData = {}
...
function OpenResellerMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elements = {
			...
			{label = TranslateCap('set_vehicle_job_owner_sell'),     value = 'set_vehicle_job_owner_sell'},
		...
		elseif action == 'set_vehicle_job_owner_sell' then
			if currentDisplayVehicle then
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					vehicleProps.plate = newPlate
					SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
					TriggerServerEvent('esx_vehicleshop:setVehicleOwnedPlayerJob', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					currentDisplayVehicle = nil
				else
					ESX.ShowNotification(TranslateCap('no_players'))
				end
...
function hasEnteredMarker(zone)
	...
	elseif zone == 'ShopJobEntering' then
		if not Config.EnablePlayerManagement then
			CurrentAction     = 'shop_job_menu'
			CurrentActionMsg  = TranslateCap('shop_job_menu')
			CurrentActionData = {}
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'shop_job_menu' then
					if ESX.PlayerData.job.grade_name == 'boss' then
						OpenShopJobMenu()
					end
