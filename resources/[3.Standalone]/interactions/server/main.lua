...
lib.callback.register('interactions:handcuff', function(source, target, playerheading, playerlocation, playerCoords)
	...
	if (xPlayer.job.name == 'police') or (hasHandCuff.count > 0) then
		TriggerClientEvent('interactions:handcuff', target, playerheading, playerlocation, playerCoords)
		return true
	elseif (xPlayer.faction.name == 'ballas') or (xPlayer.faction.name == 'vagos') or (xPlayer.faction.name == 'aztecas') or (xPlayer.faction.name == 'lost') or (xPlayer.faction.name == 'mafia') or (xPlayer.faction.name == 'cartel') or (xPlayer.faction.name == 'marabunta') or (xPlayer.faction.name == 'families') or (hasZiptie.count > 0) then
		TriggerClientEvent('interactions:handcuff', target, playerheading, playerlocation, playerCoords)
		return true
	else
		TriggerClientEvent('ox_lib:notify', source, {
			type = 'error',
			description = "Vous avez besoin de menottes ou de zips"
		})
		return false
	end
end)
